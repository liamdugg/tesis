...
lab = np.loadtxt(MEAS_DIR / "LAB_CNEA" / "parsed_data" / "s31_v2.dat", skiprows=1)
ads = np.loadtxt(MEAS_DIR / "ADS"      / "parsed_data" / "s31.dat"   , skiprows=1)
cst = np.loadtxt(MEAS_DIR / "CST"      / "parsed_data" / "s31.dat"   , skiprows=1)
...
ax.set_ylabel(r"$|S_{31}|$ (dB)" , labelpad=3)
...
fig.savefig(OUTPUT_DIR / "coupler_s31_lab_ads_cst.png", format='png')
print(f"\nGuardado\n")
...
A partir de las componentes de continua y de la fundamental, la eficiencia de drain del amplificador, definida como el cociente entre la potencia de RF entregada a la carga y la potencia continua consumida, queda determinada en función del ángulo de conducción mediante la expresión:

% TODO: cita al que cita Razavi
\begin{equation}
...
donde $\Gamma_S$ y $\Gamma_L$ son los coeficientes de reflexión de la fuente y de la carga, respectivamente, referidos a $Z_0$. La ecuación~\eqref{eq:ganancia_transductora} pone de manifiesto que la ganancia de un amplificador no depende exclusivamente de las propiedades intrínsecas del transistor, sino también de las impedancias presentadas en sus puertos de entrada y de salida, las cuales son determinadas por las redes de adaptación diseñadas.

En el caso particular en que tanto la fuente como la carga se encuentran adaptadas a $Z_0$ ($\Gamma_S = \Gamma_L = 0$), condición habitual de caracterización mediante un analizador de redes vectorial, la ecuación~\eqref{eq:ganancia_transductora} se reduce a:
...
% -----------------------------------------------------------------------------------------------%
% OIP3 cita \cite[pp.~515--517]{Pozar2012}
...
Para analizar el origen de estos productos, la característica de transferencia no lineal del amplificador puede aproximarse mediante un desarrollo en serie de potencias de la tensión de entrada $v_{in}(t)$, truncado hasta el tercer orden:
...
Sustituyendo la ecuación~\eqref{eq:dos_tonos} en~\eqref{eq:serie_potencias} y desarrollando el término cúbico mediante identidades trigonométricas, se obtiene un conjunto de componentes espectrales en distintas combinaciones de $\omega_1$ y $\omega_2$. Entre ellas, el término $a_3\,v_{in}^3(t)$ da origen, en particular, a componentes en las frecuencias $2\omega_1 - \omega_2$ y $2\omega_2 - \omega_1$, cuya amplitud resulta:
...
Cripps \cite{cripps2006} señala que, para amplificadores de potencia con características de transferencia razonablemente suaves, el OIP3 se relaciona empíricamente con el P1dB mediante un factor aproximadamente constante, de modo que:
...
% TODO: revisar y agregar citas
\subsection{Distorsión AM-AM y AM-PM}
Adicionalmente a la distorsión de amplitud descripta hasta aquí, las no linealidades del dispositivo dan lugar a la \textbf{conversión AM-AM y AM-PM}, fenómenos mediante los cuales una variación en la amplitud de la señal de entrada produce, respectivamente, una variación no proporcional en la amplitud de la señal de salida y una variación indeseada en su fase instantánea. La conversión AM-AM se relaciona directamente con la compresión de ganancia ya descripta, mientras que la conversión AM-PM no tiene un correlato evidente en las curvas de potencia de entrada y salida, dado que se manifiesta exclusivamente en la fase de la señal. Estos efectos resultan particularmente relevantes para esquemas de modulación con envolvente no constante, dado que las variaciones de amplitud propias de dichas modulaciones se traducen, al atravesar una etapa con conversión AM-AM y AM-PM apreciable, en distorsión de la constelación recibida y en regeneración espectral fuera de la banda asignada al canal.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Diseño y Optimización de Amplificadores de Potencia}
El diseño de la etapa de amplificación no se limita a la selección del transistor y de su punto de polarización, sino que requiere adicionalmente el diseño de las redes de adaptación de impedancia que vinculan al dispositivo con la fuente de excitación y con la carga. La \textbf{adaptación de impedancia} consiste en interponer, entre dos puertos de impedancias distintas, una red circuital, típicamente reactiva e idealmente sin pérdidas, que transforma una de dichas impedancias en el conjugado complejo de la otra, o bien en algún otro valor de interés según el criterio de diseño adoptado, con el objetivo de controlar la transferencia de potencia entre ambos puertos.

\subsection{Adaptación de Entrada}
En el puerto de entrada del amplificador, el objetivo de diseño es maximizar la transferencia de potencia disponible desde la fuente de excitación hacia el gate del transistor. Este objetivo se alcanza mediante el teorema de máxima transferencia de energía, según el cual la potencia entregada a una carga es máxima cuando la impedancia de dicha carga es igual al complejo conjugado de la impedancia de la fuente que la excita. Aplicado al puerto de entrada del amplificador, esto implica diseñar la red de adaptación de entrada de modo que la impedancia que ve el generador sea igual al conjugado complejo de la impedancia de fuente $Z_g$, es decir, $Z_{in} = Z_S^*$. Dado que en el puerto de entrada el objetivo es únicamente maximizar la potencia entregada al dispositivo sin que existan consideraciones adicionales de gran señal, la adaptación conjugada resulta el criterio de diseño apropiado y suficiente para esta red.

\subsection{Adaptación de Salida}
El criterio de adaptación conjugada, sin embargo, no resulta aplicable de la misma manera en el puerto de salida del amplificador. En pequeña señal, la impedancia de carga que maximiza la transferencia de potencia hacia la carga sí corresponde al complejo conjugado de la impedancia de salida del transistor. No obstante, un amplificador de potencia opera en gran señal y su comportamiento se aparta del descripto por los parámetros S. En estas condiciones, la impedancia de carga que maximiza la potencia de salida y la eficiencia del amplificador no coincide, en general, con el complejo conjugado de $S_{22}$, dado que este último se mide bajo excitación reducida y no captura el comportamiento no lineal del transistor en gran señal.

Puede demostrarse que, para un transistor cuya corriente de drain se aproxima mediante el modelo de ángulo de conducción presentado en la Sección~\ref{sec:funcionamiento_clases}, la resistencia de carga óptima que permite la máxima excursión simultánea de tensión y corriente en el drain, y que por lo tanto maximiza la potencia de salida en gran señal, se aproxima mediante:

\begin{equation}
    R_{L,opt} \approx \frac{2\,(V_{DD} - V_{knee})}{I_{max}}
    \label{eq:rl_optima}
\end{equation}

donde $V_{DD}$ es la tensión de alimentación de drain, $V_{knee}$ es la tensión de "rodilla" del dispositivo, por debajo de la cual la corriente de drain deja de responder linealmente a la tensión aplicada, e $I_{max}$ es la corriente máxima de drain. La ecuación~\eqref{eq:rl_optima} muestra que la impedancia de carga óptima depende exclusivamente de los límites de excursión de tensión y corriente del dispositivo, y no de sus parámetros $S$ de pequeña señal, lo cual confirma que el criterio de adaptación conjugada no resulta aplicable al diseño de la red de salida de un amplificador de potencia.

\subsection{Método de Load-Pull}
Dado que la expresión~\eqref{eq:rl_optima} constituye únicamente una aproximación, que no contempla efectos como las capacitancias parásitas, armónicos en la impedancia de carga ni las no linealidades de orden superior de la corriente de drain, la impedancia óptima empleada en el diseño práctico se determina habitualmente mediante la técnica de \textbf{load-pull}. Esta técnica consiste en variar la impedancia de carga presentada al puerto de salida del transistor, manteniendo fijo el nivel de excitación de entrada, y registrar en cada punto del barrido el desempeño resultante del dispositivo.

El resultado de un barrido de load-pull se representa habitualmente mediante contornos trazados sobre el diagrama de Smith, donde cada contorno conecta los puntos de impedancia de carga que producen un mismo valor de una magnitud de interés, típicamente la potencia de salida ($P_{out}$) o la eficiencia de potencia agregada (PAE). Estos contornos permiten identificar, para un nivel de excitación dado, la región del diagrama de Smith donde el dispositivo alcanza su máxima potencia de salida y la región donde alcanza su máxima eficiencia, regiones que, en general, no coinciden entre sí. La impedancia de carga de diseño se selecciona entonces como un compromiso entre ambos criterios, priorizando la intersección de los contornos donde ambas magnitudes se mantienen simultáneamente próximas a sus valores máximos, en lugar de optimizar exclusivamente alguna de las dos de forma aislada.

\section{Conclusiones del Capítulo}
A lo largo del capítulo se desarrollaron los fundamentos teóricos necesarios para abordar el diseño de un amplificador de potencia de RF. El análisis del ángulo de conducción permitió establecer que la clase de operación de un amplificador implica un compromiso entre eficiencia, potencia de salida y linealidad, dado que la reducción del ángulo de conducción mejora la eficiencia a expensas de un mayor contenido armónico y de una menor potencia disponible en la componente fundamental. Este compromiso constituye el criterio fundamental para la selección de la clase de polarización del transistor.

Asimismo, se estableció que la caracterización de pequeña señal, si bien resulta indispensable para evaluar la estabilidad y la ganancia lineal del dispositivo, no es suficiente para predecir su comportamiento en el régimen de excitación de interés para un amplificador de potencia. Los parámetros de gran señal, en particular el punto de compresión de 1 dB, la potencia de saturación y las distintas métricas de eficiencia, resultan indispensables para caracterizar el desempeño real del dispositivo en su punto de operación.

De manera análoga, se explicaron los mecanismos más relevantes que dan origen a la distorsión no lineal del amplificador, estableciendo al OIP3 como figura de mérito y su vinculación empírica con el P1dB. Se destacó que la conversión AM-AM y AM-PM introduce distorsión adicional de particular relevancia para esquemas de modulación con envolvente no constante, como los empleados en el presente trabajo.

Finalmente, se discutió que el diseño de las redes de adaptación de impedancia no puede abordarse mediante un único criterio aplicable indistintamente a ambos puertos del amplificador. Mientras que la adaptación de entrada admite el criterio de máxima transferencia de potencia mediante adaptación conjugada, la adaptación de salida requiere de la determinación experimental o simulada de la impedancia óptima de carga mediante la técnica de load-pull.

En síntesis, el diseño de un amplificador de potencia constituye un problema de optimización de múltiples variables, en el cual la potencia de salida, la eficiencia, la linealidad y la estabilidad se encuentran mutuamente acopladas, y en el cual ninguna de estas prestaciones puede maximizarse de manera aislada sin afectar a las restantes. Los criterios y herramientas presentados en este capítulo constituyen el marco teórico sobre el cual se fundamentan las decisiones de diseño del amplificador desarrollado en capítulos posteriores.